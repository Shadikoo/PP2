...
while running:
    ...
    x = 20
    for tool in tools:
        button = pygame.Rect(x, 20, 100, 40)

        if tool == current_tool:
            pygame.draw.rect(screen, HIGHLIGHT, button)
        else:
            pygame.draw.rect(screen, LIGHT_GRAY, button)

        pygame.draw.rect(screen, BLACK, button, 2)

        text = font.render(tool.upper(), True, BLACK)
    ...
    for color in colors:
        ...
        if color == current_color:
            pygame.draw.rect(screen, HIGHLIGHT, color_button, 3)
        else:
    ...
    if drawing and current_tool in ["rect", "circle"]:
        ...
        if current_tool == "rect":
            pygame.draw.rect(preview, current_color, shape_rect, 2)

        if current_tool == "circle":
            pygame.draw.ellipse(preview, current_color, shape_rect, 2)
    ...
    for event in pygame.event.get():
            ...
            if drawing:
                ...
                if mouse_y > TOOLBAR:
                    ...
                    if current_tool == "brush":
                        pygame.draw.line(canvas, current_color, last_pos, new_pos, 4)

                    if current_tool == "eraser":
                        pygame.draw.line(canvas, WHITE, last_pos, new_pos, 20)
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if drawing:
                ...
                if current_tool == "rect":
                    pygame.draw.rect(canvas, current_color, shape_rect, 2)

                if current_tool == "circle":
                    pygame.draw.ellipse(canvas, current_color, shape_rect, 2)
